...
" [Commenting this out since I had to take the backup (aka, the github dotfile local repository
" copy) since there was some inexplicable reason a lot of the file got deleted].
" [ Not many changes were made aside from cleaning up the command for inserting bash file preambles,
" as well as adding--or rather fixing the one for Python 3]
" 
" (Changes/reversion made on: [5-5-2022 8PM]):
"
...
" Function for python3 script file preamble info.
	function! Python3Header(...)
		...
		cnoreabbrev py3h silent! call Python3Header()
		cnoreabbrev p3h silent! call Python3Header()
...
fu! NewTempFile(...)
	...
	let l:datetime = system("date '+%m-%d-%Y_%I-%M%p'")			"Contains date with seconds with AM/PM.
	let l:directory = "/tmp/tempfiles/vim-tempfiles/"
	let l:filename = echo expand(l:datetime)
	"echom (expand(g:something) . expand(g:else))
