...
"	(Too braindead to figure out right now, come back to this later).
	function DeleteCurrentOpenFile()
		...
		if l:confirm == 1
			...
			sleep 2
			silent! exe "call delete(expand('%:p')) | bdelete! | norm <CR>"
			silent! exe "quit"
		"elseif l:confirm == 0
		else
...
fu! NewTempFile(...)
	...
	let l:datetime = system("date '+%m-%d-%Y___%H-%M-%S'")
	let l:filename = "tempfile___" . l:datetime
...
"	endif
	...
	if get(a:, 0) > 0
		...
		echom "Default filename will be used in the format of: <MM-DD-YYYY___HH-MM-SS> in the '/tmp/vim-tmpfiles/' directory."
		let l:filename = "/tmp/nvim-tempfiles/" . expand(l:datetime)
		exe "e " . l:directory . l:filename
	endif

endfu!

command -nargs=? NTF call NewTempFile(<q-args>)
cnoreabbrev ntf NTF
